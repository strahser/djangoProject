"""API конвейера РД для ИИ-агента (M1 DOC-3, канон §5).

Агент ходит только сюда, не в БД напрямую. Каждый POST пишет DocChangeLog source='api'.
...
from .serializers import DocRegisterEntrySerializer, DocRevisionSerializer
...
def _log(*, entry=None, revision=None, field, old='', new='', user=None):
...
def intake(request):
    """Шаг 1: приём вложений письма → DocRevision(received). Тело: {email_id, attachment_id?}."""
    ...
    rev = DocRevision.objects.create(
        entry=None, email=email, attachment=att, source='email',
        sha256=sha, size=size, status='received',
    )
    _log(revision=rev, field='intake', old='', new=f'email={email.pk} att={att.pk if att else None}', user=request.user)
    return Response(DocRevisionSerializer(rev).data, status=status.HTTP_201_CREATED)


@api_view(['POST'])
@permission_classes([IsAuthenticated])
def validate(request, rev_id):
    """Шаг 2: валидация движком DesignBase → DocCheck + статус checked_ok/has_remarks.

    FAIL если Подано.verdict == NO_CODE (работа не подана — канон П2).
    """
    rev = get_object_or_404(DocRevision, pk=rev_id)
    ...
    prev = DocRevision.objects.filter(
    ...
    try:
        ...
        if code and not cipher:
            cipher = DocRegisterEntry.objects.filter(code=code).values_list('cipher', flat=True).first()
        if cipher:
    ...
    DocCheck.objects.update_or_create(
        revision=rev,
    ...
    rev.save(update_fields=['status'])
    _log(entry=rev.entry, revision=rev, field='validate', old=old_status,
         new=f'{rev.status} ({verdict})', user=request.user)
    return Response({'revision': rev.pk, 'status': rev.status, 'verdict': verdict,
                     'questions': questions, 'checks': checks})


@api_view(['POST'])
@permission_classes([IsAuthenticated])
def register(request, rev_id):
    """Шаг 3: запись в реестр. Тело: {code?} — существующий код либо новый (confirm при новом шифре)."""
    rev = get_object_or_404(DocRevision, pk=rev_id)
    ...
    if code:
        ...
        if not confirm:
            return Response({'error': 'новая запись реестра — нужен confirm:true',
                             'next_code': next_code()}, status=400)
        entry = DocRegisterEntry.objects.create(code=next_code())
    if rev.entry_id != entry.pk:
        old = rev.entry.code if rev.entry else None
        rev.entry = entry
        rev.save(update_fields=['entry'])
        _log(entry=entry, revision=rev, field='entry', old=old, new=entry.code, user=request.user)
    # номер ревизии в рамках записи
    if rev.rev_no <= 0:
        rev.rev_no = 1
    last = DocRevision.objects.filter(entry=entry).exclude(pk=rev.pk).order_by('-rev_no').first()
    if last and rev.rev_no <= last.rev_no:
        rev.rev_no = last.rev_no + 1
        rev.save(update_fields=['rev_no'])
    rev.status = 'registered'
    rev.save(update_fields=['status'])
    _log(entry=entry, revision=rev, field='register', old='', new=entry.code, user=request.user)
    return Response({'revision': rev.pk, 'entry': entry.code, 'rev_no': rev.rev_no, 'status': rev.status})


@api_view(['POST'])
@permission_classes([IsAuthenticated])
def remarks(request, rev_id):
    """Шаг 4а: замечание + (позже) лист согласования. Тело: {text}."""
    rev = get_object_or_404(DocRevision, pk=rev_id)
    text = (request.data.get('text') or '').strip()
    if not text:
        return Response({'error': 'нужен text'}, status=400)
    rm = DocRemark.objects.create(revision=rev, text=text)
    old = rev.status
    rev.status = 'has_remarks'
    rev.save(update_fields=['status'])
    _log(entry=rev.entry, revision=rev, field='remarks', old=old, new=f'#{rm.pk}', user=request.user)
    return Response({'remark': rm.pk, 'status': rev.status}, status=201)


@api_view(['POST'])
@permission_classes([IsAuthenticated])
def issue(request, rev_id):
    ...
    rev.status = 'issued'
    rev.save(update_fields=['status'])
    _log(entry=rev.entry, revision=rev, field='issue', old='registered', new=no, user=request.user)
...
@permission_classes([IsAuthenticated])
def queue(request):
    """Очередь конвейера по статусам для агента."""
    out = {}
...
@api_view(['GET'])
@permission_classes([IsAuthenticated])
def entry_card(request, code):
    """Карточка записи: ревизии, проверки, замечания, выдачи, связи."""
    entry = get_object_or_404(DocRegisterEntry, code=int(code))
    revs = []
    ...
    data = DocRegisterEntrySerializer(entry).data
